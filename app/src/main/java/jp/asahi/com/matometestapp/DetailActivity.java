...
public class DetailActivity extends AppCompatActivity implements TabClick {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int number = getIntent().getIntExtra(HomeActivity.EXTRA_ITEM_NUM, -1);

        ImageView imageView = (ImageView) findViewById(R.id.detail_activity_iamge);
        TextView textView = (TextView) findViewById(R.id.detail_activity_price);

        imageView.setImageResource(TopActivityItemList.images[number]);
        String s = "";
        int price = TopActivityItemList.prices[number];
        while (price > 0) {
            if (price < 1000) {
                s = price + s;
                price = 0;
